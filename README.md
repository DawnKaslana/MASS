...
```math
L(\theta; \mathcal X) = \frac{1}{|\mathcal X|} \sum_{x \in \mathcal X}
```
```math
 \log P(x^{u:v} | x^{\setminus u:v}; \theta) = \frac{1}{|\mathcal X|} \log \Pi^{v}_{t=u} P(x^{u:v}_{t}|x^{u:v}_{<t}, x^{\setminus u:v};\theta)
```
